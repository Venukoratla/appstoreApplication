...
class AppStore extends Component {
  state = {tabIdIs: tabsList[0].tabId, searchInput: ''}
  ...
  getfilteredList = () => {
    const {tabIdIs} = this.state
    const filteredList = appsList.filter(
      eachItem => eachItem.category === tabIdIs,
    )
    return filteredList
  }

  changeId = tabId => {
    this.setState({tabIdIs: tabId})
  }

  render() {
    const filteredList = this.getfilteredList()
    const {tabIdIs, searchInput} = this.state

    const filteredSearchResults = filteredList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div>
        <h1>App Store</h1>
        <input type="search" onChange={this.searchValue} value={searchInput} />
        <img
        ...
        <ul>
          {filteredSearchResults.map(eachApp => (
